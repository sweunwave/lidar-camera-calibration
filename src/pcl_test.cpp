...
int main(int argc, char **argv) {
    ...
    *ptr_cloud = cloud2;
    ...
    // 3: 10, 11, 12
    // 4: -1, -2, -3
    // 위의  ptrcloud2 = ptrcloud; 의 결과로, ptr_cloud2와 ptr_cloud가 가리키는 points가 같다!!     
    cout << " ---------------- " << endl;

    // 함수 인자로 받을때: 둘다 위에서 출력한 주소와 같은 주소를 출력함
    // 위에서 출력한 것과 같은 주소값(0x55b586287000)을 print함
    ...
    // $Ptr$ = $Ptr$과는 다르게 *$Ptr$ = *$Ptr$을 하면 메모리에 있던 정보를 복사해서 넘겨준다는 의미임
    ...
    *ptr_cloud3 = *ptr_cloud;
    ...
    // 6: 1, 0, 0
    cout << " ------ $Ptr$ = $Ptr$ ------ " << endl;
    // ptr_cloud2는 이전에 $Ptr$ = $Ptr$를 통해 한 객체를 공유하고 있음
